...
const PanVerificationDialog = ({
	...
	useEffect(() => {
		if (verifyPanData) {
			...
			if (verifyPanData.success) {
				...
				const verifiedName =
					responseData.registered_name || responseData.full_name || responseData.name;
		...
		if (verifyPanError) {
			...
			const errorMsg =
				verifyPanError.data?.message || verifyPanError.message || t("Failed to verify PAN");
	...
	const handleSubmitVerification = useCallback(() => {
		...
		const effectiveName =
			responseData.registered_name || responseData.full_name || responseData.name || localName;
				...
				>
					<View style={styles.modalContent}>
						...
						<PoppinsTextMedium
							...
							content={
								isPreVerified ? t("PAN Already Verified") : t("Kindly Enter Your PAN Details")
							}
						/>
